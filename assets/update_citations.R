...
rs <- yaml.load_file("~/Dropbox/research/scripts/websites/guillaumelobet.github.io/_data/publications.yml")
cites <- c()
  ...
  if(length(rs[[i]]$doi) > 0){
   print(rs[[i]]$doi) 
    tryCatch({
      ...
      message(paste0(doi, ":", rs[[i]]$citations))
    }, warning = function(w) {
...
cat(as.yaml(rs, precision = 3, indent = 3), 
    file=paste("~/Dropbox/research/scripts/websites/guillaumelobet.github.io/_data/publications.yml",sep=""), sep='\n') # Create the input file for Archisimple
